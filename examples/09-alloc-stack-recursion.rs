...
fn init_heap() {
    const HEAP_SIZE: usize = 32 * 1024; // Define heap size
...
fn recursive_stack_and_heap_allocation(depth: usize, allocation_size: usize) {
    ...
    info!("Depth: {}, Stack usage: {} bytes, Heap allocation: {} bytes",
          depth, depth * 1024, allocation_size);

    if depth > 1000 {
        return; // Limit recursion depth to prevent stack overflow
    }

    recursive_stack_and_heap_allocation(depth + 1, allocation_size + 1024); // Increase allocation size each recursion
}
